...
cmd_enter() {
  ...
  # Write a real rcfile inside the chroot at /tmp/.ros2-chroot-rc.<pid>
  # (i.e. ${ROOTFS_DIR}/tmp/... from the host's perspective). Using a real
  # file -- instead of bash process substitution <(...) -- matters because
  # process substitution creates /dev/fd/N entries scoped to the host bash,
  # which the chrooted shell cannot open.
  local rc_outside="${ROOTFS_DIR}/tmp/.ros2-chroot-rc.$$"
  local rc_inside="/tmp/.ros2-chroot-rc.$$"
  mkdir -p "${ROOTFS_DIR}/tmp"
  cat > "${rc_outside}" <<EOF
# Auto-generated by ros2-chroot.sh
...
cd ${ROS_WORKSPACE}
EOF
  chmod 0644 "${rc_outside}"
  # Suppress Ubuntu's "use sudo <command>" first-time hint for ros2.
  ...
  #
  # 'chroot --userspec' switches to the ros2 user/group directly, avoiding
  # the 'su -c' indirection that drops job control and prints the noisy
  # "cannot set terminal process group / no job control in this shell"
  # warnings on entry.
  /usr/sbin/chroot --userspec="${ROS_USER}:${ROS_USER}" "${ROOTFS_DIR}" \
    /usr/bin/env -i \
      HOME="/home/${ROS_USER}" \
