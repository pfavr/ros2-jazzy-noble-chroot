...
default_overlay_setups="/opt/ros2_ws/install/setup.bash:/overlay_ws/install/setup.bash:/opt/app_ws/install/setup.bash:/workspaces/ros2_ws/install/setup.bash"
...
shopt -s nullglob
for hook in /usr/local/share/ros2-layer-entrypoint.d/*.sh; do
  if [[ -r "${hook}" ]]; then
    # shellcheck disable=SC1090
    source "${hook}"
  fi
